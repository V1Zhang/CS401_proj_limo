...
# 创建一个Publisher，发布名为/identify_info的topic，消息类型为limo_deeplearning::identify，队列长度3
    identify_info_pub = rospy.Publisher('/identify_info', identify, queue_size=3)

# 设置循环的频率
    rate = rospy.Rate(10) 
    global image_numbers
    # 初始化limo_deeplearning::identify类型的消息
    identify_msg = identify()
    identify_msg.results = names;          # 检测结果（字符）
    identify_msg.classes  = classes;       # 检测类别（数字）
    identify_msg.area  = number;           # 识别框面积
    identify_msg.position = position;      # 识别框左上顶点，右下顶点像素坐标（xyxy）
    identify_msg.acc = acc;                # 识别精确度
    identify_msg.image_number = image_numbers;   # 当前识别为第多少张图像
    # 发布检测结果
    identify_info_pub.publish(identify_msg)
    print (position)
    rospy.loginfo("Publish identifiy message [%s, %d, %d, %f]",
                  identify_msg.results, identify_msg.classes, identify_msg.area, identify_msg.acc)
    rate.sleep()
